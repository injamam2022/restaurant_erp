...
class OrderMangement extends CI_Controller {
	/**
	...
	public function index()
	{
        ...
        $filter_orders_admin = $api_url."General/Filterordersadmin";
        ...
        $data['paymentDetails']=json_decode($response,true);  
        
        
		$this->load->view('OrderMangement/cssLinks');
		$this->load->view('Template/header');
		$this->load->view('Template/slider');
		$this->load->view('OrderMangement/mainSection',$data);
		$this->load->view('Template/footer');
		$this->load->view('OrderMangement/jqueryLinks');
	}
    ...
    function download()
    {
      ...
      $data= glob('./master_images/Invoice/'.date('Y').'/'.$month . "/*.pdf");
      ...
      $this->zip->download(''.date('Y').'-'.$month.'.zip');
    }

    public function exports_data(){
        $api_url = $this->config->item('api_url');
        $get_api_url = $api_url."General/GetAllSaleReport";
        $objarray = $this->input->post();
        $data = $this->General_model->general_function($objarray,$get_api_url);
        echo  $data;
    }
    ...
    public function viewinvoice(){
      ...
      $api_url = $this->config->item('api_url');
      $get_api_url = $api_url."General/GetallordersgroupInfo";
      mkdir('./master_images/Invoice/'.date('Y').'/'.date('M'), 0777, TRUE);

      $objarray = array("order_id"=>shortURLtoID($order_id));
      $dataorder_info['order_invoice_details'] = json_decode($this->General_model->general_function($objarray,$get_api_url),true);

      
      if($dataorder_info['order_invoice_details'][0]['status_id'] == 2){
        $invoice_no = $dataorder_info['order_invoice_details'][0]['bill_invoice_number'];
        require_once __DIR__ . '/mpdf/vendor/autoload.php';
        // Create an instance of the class:
        ob_clean();
        $mpdf = new \Mpdf\Mpdf();
        // Write some HTML code:
        $string = $this->load->view('OrderMangement/invoice',$dataorder_info, TRUE); 
        $mpdf->SetWatermarkText('PAID','0.3');
        $mpdf->showWatermarkText = true;

        $mpdf->WriteHTML($string);
        
        //$mpdf->Output();
        $mpdf->Output('./master_images/Invoice/'.date('Y').'/'.date('M').'/'.$invoice_no.'.pdf','F');
        // Output a PDF file directly to the browser
      }
     
      $this->load->view('OrderMangement/invoice',$dataorder_info);
    }

    public function viewKitcheninvoice(){
      $order_id = $this->uri->segment(3);
      $api_url = $this->config->item('api_url');
      $get_api_url = $api_url."General/GetallordersgroupKitchenInfo";
      $update_api_url = $api_url."General/Update";
      
      $objarray = array("order_id"=>shortURLtoID($order_id));
      $dataorder_info['order_invoice_details'] = json_decode($this->General_model->general_function($objarray,$get_api_url),true);
      
      $objarray = array("order_details"=>array("kitchen_status"=>1,"kitchen_quantity"=>0),"where"=>array("order_id"=>shortURLtoID($order_id)));
      $response = $this->General_model->general_function($objarray,$update_api_url);

      $this->load->view('OrderMangement/invoice_kitchen',$dataorder_info);
    }
    ...
    public function delete_order(){
      $api_url = $this->config->item('api_url');
      $update_api_url = $api_url."General/DeleteOrder";
